...
class AmzFindBoxScreen extends StatefulWidget {
  const AmzFindBoxScreen({super.key});

  @override
  _AmzFindBoxScreenState createState() => _AmzFindBoxScreenState();
}

class _AmzFindBoxScreenState extends State<AmzFindBoxScreen>
  ...
  void initState() {
    super.initState();
    _searchIconController = AnimationController(
      vsync: this,
  ...
  Future<void> _searchBox() async {
    ...
    try {
      ...
      final res = await api.client.post(
        '/amz/grading/search_box',
        jsonBody: {'box_name': _controller.text.trim()},
      );
      if (res.ok && res.body is Map) {
        ...
        setState(() {
          _error = res.error ?? 'Error al buscar caja';
        });
      }
    } catch (e) {
      setState(() {
  ...
  Future<void> _deleteBox(String boxName) async {
    try {
      final api = Provider.of<ApiService>(context, listen: false);
      final payload = {'box_name': boxName};
      ...
      final res = await api.client.post(
        '/amz/grading/delete_box',
        jsonBody: payload,
      );
      if (res.ok) {
        setState(() {
          _results.removeWhere((r) => r['box_name'] == boxName);
        });
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              res.body is Map
                  ? (res.body['message'] ?? 'Eliminado')
                  : 'Eliminado',
            ),
          ),
        );
  ...
  Future<void> _reprintLabel(String boxName) async {
    try {
      final api = Provider.of<ApiService>(context, listen: false);
      final res = await api.client.post(
        '/amz/grading/reprint_label',
        jsonBody: {'box_name': boxName},
      );
      final msg = (res.body is Map)
          ? (res.body['message'] ?? 'Etiqueta reimpresa')
          : 'Etiqueta reimpresa';
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(msg)));
    } catch (_) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Error reimprimiendo etiqueta')),
      );
    }
  }
  ...
  void _showDetails(Map<String, dynamic> item) {
    showDialog(
      ...
      builder: (_) => SelectionArea(
        child: AlertDialog(
          title: Text(
            'Detalle de la Caja ${item['box_name']}',
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _buildDetailRow(
                  'Fecha de puesta',
                  _formatDate(item['put_time']),
                ),
                _buildDetailRow(
                  'Usuario que inserto la box',
                  item['username_put'],
                ),
                _buildDetailRow(
                  'Fecha de OPS-Pick',
                  _formatDate(item['opspick_time']),
                ),
                _buildDetailRow('Usuario OPS-Pick', item['username_opspick']),
                _buildDetailRow(
                  'Fecha de cierre',
                  _formatDate(item['box_close']),
                ),
                _buildDetailRow(
                  'Usuario que cerró la box',
                  item['username_close'],
                ),
              ],
            ),
  ...
  Widget _buildDetailRow(String etiqueta, dynamic valor) {
    final texto = (valor == null || valor.toString().isEmpty || valor == 'null')
        ? '—'
        : valor.toString();
    return Padding(
                            ...
                            children: [
                              ...
                              if (_results.isNotEmpty)
                                Padding(
                                  ...
                                  child: Text(
                                    'Total de unidades en la caja: ${_results.length}',
                                    style: TextStyle(
                                      color: theme.colorScheme.onSurface,
