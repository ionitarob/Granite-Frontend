...
class WidgetGrid extends StatefulWidget {
  ...
  final Map<String, int>? spanColumns; // optional: widgetId -> number of columns to span

  const WidgetGrid({Key? key, required this.availableWidgets, required this.storageKey, this.spanColumns}) : super(key: key);
...
class _WidgetGridState extends State<WidgetGrid> {
  ...
  Future<void> _loadLayout() async {
    ...
    if (raw != null) {
      try {
        ...
        setState(() => _layout = parsed.map((e) => e == null ? null : e.toString()).toList());
        return;
      } catch (_) {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      while (i < _layout.length) {
        ...
        if (widgetId != null && widget.availableWidgets.containsKey(widgetId) && span > 1) {
          // make spanning item width
          final widthSpan = itemWidth * span + (span - 1) * 12;
          children.add(_buildDraggableItem(i, widgetId, widthSpan, span: span));
          i += span;
        } else if (widgetId != null && widget.availableWidgets.containsKey(widgetId)) {
  ...
  Widget _buildDraggableItem(int index, String widgetId, double width, {int span = 1}) {
    ...
    return SizedBox(
      ...
      child: LongPressDraggable<String>(
        ...
        feedback: Material(
          ...
          child: SizedBox(width: width, child: Opacity(opacity: 0.95, child: w)),
        ),
        childWhenDragging: Container(
          height: 90,
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.white.withAlpha(15))),
          child: const Center(child: Text('Moving...')),
        ),
        child: DragTarget<String>(
          onWillAcceptWithDetails: (details) => details.data != widgetId,
          onAcceptWithDetails: (details) => _moveWidget(details.data, index, span: span),
        ...
        builder: (context, candidate, rejected) => Container(
          ...
          child: Center(
            child: candidate.isNotEmpty
                ...
                : const Text('Empty slot', style: TextStyle(color: Colors.white70)),
          ),
        ),
      ),
    );
