...
def prefix_search():
    prefixes = ['CQ', 'CP', 'CM', 'CL']
    print(f"Searching for SKUs in [datapass].[administracion].[cotizaciones] starting with: {prefixes}")
    
    with connection.cursor() as cur:
        for p in prefixes:
            print(f"\n--- Results for {p}% ---")
            query = """
                SELECT TOP 10 sku_config, sku_hp, sku_lenovo, description 
